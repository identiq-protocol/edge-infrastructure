...
module "network" {
  ...
  subnet_names        = var.support_slb_feature ? ["subnet1", "subnet2", "subnet3", "subnet4"] : ["subnet1", "subnet2", "subnet3"]
...
resource "azurerm_subnet_nat_gateway_association" "nat_gw_a" {
  nat_gateway_id = azurerm_nat_gateway.nat_gw.id
  subnet_id      = var.support_slb_feature ? module.network.vnet_subnets[3] : module.network.vnet_subnets[0]
}
...
resource "azurerm_kubernetes_cluster_node_pool" "db" {
  ...
  orchestrator_version  = var.aks_kubernetes_version
  vnet_subnet_id        = var.support_slb_feature ? module.network.vnet_subnets[3] : module.network.vnet_subnets[0]
  node_labels = {
